<?php
    ...
    $con = new Conexion();
    $connect = $con->get_conexion();*/
    require('config/variables_BD.php');
    $constants= array();
    $constants[]=DB_HOST;
    $constants[]=DB_USUARIO;
    $constants[]=DB_CONTRA;
    $constants[]=DB_NOMBRE;   
    $constants[]=DB_CHARSET; 

    //$contantes = $_POST["constants"]
    $connect = mysqli_connect($constants[0], $constants[1], $constants[2], $constants[3]);//Configurar los datos de conexion
    ...
    //$query = 'SELECT bitacora.*, usuarios.*, permisos.*, sistemas.* FROM bitacora JOIN USUARIOS on bitacora.id_usuario = usuarios.id_usuario JOIN PERMISOS  ON permisos.id_permiso = bitacora.id_permiso JOIN SISTEMAS ON sistemas.id_sistema = bitacora.id_sistema WHERE ';
    //$query .= ' WHERE ';

    if($_POST["is_date_search"] == "yes")
    {
        $query .= 'WHERE Bi.fecha_registro BETWEEN "'.$_POST["start_date"].'" AND "'.$_POST["end_date"].'" AND ';
        ...
        if(isset($_POST["nombre_usuario"])){
            $nombre_usuario = $_POST["nombre_usuario"];
            $nombre_permiso = $_POST["nombre_permiso"];
            $query .= 'WHERE Us.nombre_usuario LIKE "%'.$nombre_usuario.'%" AND Pe.nombre_permiso LIKE "%'.$nombre_permiso.'%" AND ';                 
        }
    }
